...
function ProgressIndicator({ className, value, ...props }: ProgressPrimitive.Indicator.Props & { value: number }) {
  return (
    <ProgressPrimitive.Indicator
      data-slot="progress-indicator"
      className={cn(`h-full transition-all ${value === 100 ? 'bg-green-500' : 'bg-primary'}`, className)}
      {...props}
    />
  )
}
