...
const DashboardSidebar = () => {
  ...
  const buyerLinks = [
    { to: '/dashboard', icon: LayoutDashboard, label: 'Stats Overview' },
    { to: '/dashboard/profile', icon: User, label: 'Profile' },
    { to: '/dashboard/edit-profile', icon: User, label: 'Edit Profile' },
    { to: '/dashboard/bids', icon: Gavel, label: 'My Bids' },
    { to: '/dashboard/wishlist', icon: Heart, label: 'My Wishlist' },
    { to: '/dashboard/safety', icon: ShieldCheck, label: 'Safety Tips' },
  ...
  const sellerLinks = [
    ...
    { to: '/dashboard/waiting-approval', icon: Clock, label: 'Waiting Approval' },
    { to: '/dashboard/safety', icon: ShieldCheck, label: 'Safety Tips' },
    { to: '/dashboard/settings', icon: Settings, label: 'Settings' },
  ]
      ...
      <nav className="flex-1 p-3 space-y-1">
        {links.map((link) => {
          ...
          return (
            ...
            >
              <Icon className={cn('h-4 w-4', active ? 'text-primary-600' : 'text-neutral-400')} />
              {link.label}
            </Link>
          )
        })}
